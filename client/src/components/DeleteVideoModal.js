...
import deleteBtn from '../static/img/delete-btn.svg'

const DeleteVideoModal = ({setVideoUpload, videoUpload, userId, play, setModalClose}) => {
    const [deleted, setDeleted] = useState(-1);

    useEffect(() => {
        const deleteVideoModal = document.querySelector(".modal--deleteVideo");
        deleteVideoModal.style.zIndex = '100';
        deleteVideoModal.style.opacity = "1";
    }, []);
    ...
    return <div className="modal modal--deleteVideo">
        <main className="modal__inner modal__inner--deleteVideo">
            ...
            {deleted === -1 ? <button className="button button--hover modal__inner__btn" onClick={() => { confirmDelete(); }}>
