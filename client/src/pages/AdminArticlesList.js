...
const AdminArticlesList = () => {
    ...
    const deleteArticleWrapper = () => {
        deleteArticle(candidateToDelete)
            .then((res) => {
            ...
            <div className="modal__inner">
                ...
                {deleteStatus === -1 ? <>
                    ...
                    <div className="modal__buttons">
                        <button className="modal__btn" onClick={() => { deleteArticleWrapper(); }}>
                            Usuń
                        </button>
                    ...
                    {deleteStatus === 1 ? "Artykuł został usunięty" : "Coś poszło nie tak... Prosimy spróbować później lub skontaktować się z administratorem"}
                </h3>}
            </div>
        </div>
                    ...
                    return <section className="admin__main__notification__item" key={index}>
                        <section className="admin__main__notification__item__col col-1">
                            {item.file_path ? <figure className="admin__main__notification__item__imgWrapper">
                                <img className="admin__main__notification__item__img btn__img" src={`${settings.API_URL}/image?url=/media/blog/${item.file_path}`} alt={item.title} />
                            </figure> : "-"}
                        </section>
                        ...
                        <section className="admin__main__notification__item__col col-4">
                            ...
                            <section className="admin__main__notification__item__buttons">
                                ...
                                <a className="admin__main__notification__item__btn" href={`/dodaj-artykul?id=${item.id}`}>
